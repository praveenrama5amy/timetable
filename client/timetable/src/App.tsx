...
import Button, { SplitButton } from "./assets/components/Button"
...
function App() {
    ...
    const [response, setResponse] = useState<Response>({ organisation: { name: "a", classes: [], tutors: [], timetable: {}, general: {}, subjects: [] } });
    const [viewAsSelector, setViewAsSelector] = useState<string>("Classes")
    const [viewAsSelectorSub, setViewAsSelectorSub] = useState<string>("")
    ...
    useEffect(() => {
        ...
        fetch(`${location}/api/getTimetable?organName=${organName}`, { method: "GET" }).then(async res => {
            ...
            if (response.organisation.onTemp == true) {
    ...
    function getHoursOfTheday(day: any) {
        if (timetable == null) return;
        let someObj: Object = {};
        let hours: String[] = []
        let timetable2: Timetable = timetable
        for (const e in timetable2[day]) {
            hours.push(timetable2[day][e])
        }
        return hours
    }
    ...
    async function addHour({ day, hour, value }: { day: any, hour: any, value: String }) {
        ...
        let res = (autoGenerated == false) ? await fetch(`${location}/api/addHour/${response?.organisation.name}/${viewAsSelectorSub}/${day}/${hour}/${value}`, { method: "POST" }) :
            await fetch(`${location}/api/addHour/${response?.organisation.name}/${viewAsSelectorSub}/${day}/${hour}/${value}?onTemp=true`, { method: "POST" })
    ...
    async function autoGenerate() {
        setLoading(true)
        let res: any = await fetch(`${location}/api/autoGenerate/${response?.organisation.name}`, { method: "PUT" })
        setLoading(false)
        res = await res.json()
        let respons: any = { ...response }
        res.old = respons.organisation
        respons.organisation = res;
        setResponse(respons)
        setTimeTable(res.timetable[viewAsSelectorSub])
        setAutoGenerated(true)

    }
    async function saveAutoGenerated() {
        setLoading(true)
        let res = await fetch(`${location}/api/saveAutoGenerate/${response?.organisation.name}`, { method: "PUT" })
        let response2 = await res.json()
        setResponse(response2)
        setTimeTable(response2.organisation.timetable[viewAsSelectorSub])
        setAutoGenerated(false)
        setLoading(false)
    }
    async function discardAutoGenerated() {
        setLoading(true)
        let res = await fetch(`${location}/api/autoGenerate/${response?.organisation.name}`, { method: "DELETE" })
        let response2 = await res.json()
        setResponse(response2)
        setTimeTable(response2.organisation.timetable[viewAsSelectorSub])
        setAutoGenerated(false)
            ...
            <div className="wrapper">
                <div className="viewSelector">
                    ...
                    {(autoGenerated == false) ?
                        ...
                        :
                        <>
                            <SplitButton color="success" text="Save" onClick={() => {
                                saveAutoGenerated()
                            }}>
                                <li><a className="dropdown-item" href="#" onClick={() => { autoGenerate() }}>Re-Generate</a></li>
                                <li><a className="dropdown-item" href="#" onClick={() => { discardAutoGenerated() }}>Discard</a></li>
                            </SplitButton>
                        </>
                    }
                </div>
